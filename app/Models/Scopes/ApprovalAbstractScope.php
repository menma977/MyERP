...
class ApprovalAbstractScope implements Scope
{
    ...
    public function apply(Builder $builder, Model $model): void
    {
        ...
        $builder->when(! $isAll && ! $request->input('user_id'), function (Builder $query) {
            return $query->where('created_by', Auth::id());
        })->when(! $isAll && $request->input('user_id'), function (Builder $query) use ($request) {
            return $query->where(function (Builder $query) use ($request) {
                return $query->whereHas('event.components.contributors', function (Builder $query) use ($request) {
                    return $query->where('user_id', $request->input('user_id'));
                })->orWhere('created_by', $request->input('user_id'));
            });
