<?php

namespace App\Http\Controllers\Items;

use App\Http\Controllers\Controller;
use App\Http\Resources\Items\GoodReceiptResource;
use App\Models\Items\GoodReceipt;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;
use Illuminate\Pagination\LengthAwarePaginator;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\App;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\ValidationException;

class GoodReceiptController extends Controller
{
    /**
     * Good Receipt Index
     *
     * Display a listing of the resource.
     *
     * @return LengthAwarePaginator<int, GoodReceipt>|Collection<int, GoodReceipt>|JsonResource|int
     */
    public function index(Request $request): LengthAwarePaginator|Collection|JsonResource|int
    {
        $goodReceipts = GoodReceipt::with([
            'order',
            'components',
        ])->when($request->input('search'), function (Builder $query) use ($request) {
            return $query->where(function (Builder $query) use ($request) {
                return $query->where('code', 'like', '%'.$request->input('search').'%')->orWhereHas('order', function (Builder $query) use ($request) {
                    $query->where('code', 'like', '%'.$request->input('search').'%');
                });
            });
        })->orderBy($request->input('sort_by', 'id'), $request->input('sort_order', 'desc'));

        if ($request->input('type', 'paginate') === 'collection') {
            return GoodReceiptResource::collection($goodReceipts->get());
        }

        if ($request->input('type', 'paginate') === 'count') {
            return $goodReceipts->count();
        }

        return GoodReceiptResource::collection($goodReceipts->withContributors()->withUsers()->paginate($request->input('per_page', 10), $request->input('columns', '*')));
    }

    /**
     * Good Receipt Show
     *
     * Show the specified resource.
     */
    public function show(Request $request): JsonResource
    {
        return GoodReceipt::with([
            'order',
            'components',
            'event.components.contributors.user',
            'createdBy',
            'updatedBy',
            'deletedBy',
        ])->where('id', $request->route('id'))->firstOrFail()->toResource();
    }

    /**
     * Good Receipt Update
     *
     * Update the specified resource in storage.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function update(Request $request): array
    {
        $request->validate([
            'purchase_order_id' => ['required', 'string', 'exists:purchase_orders,id'],
            'note' => ['nullable', 'string'],
        ]);

        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();
        $goodReceipt->purchase_order_id = $request->input('purchase_order_id');
        $goodReceipt->note = $request->input('note');
        $goodReceipt->save();

        return [
            'message' => trans('messages.success.update', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Delete
     *
     * Remove the specified resource from storage.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function delete(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        if ($goodReceipt->purchaseReturns()->exists()) {
            throw ValidationException::withMessages([
                'purchase_returns' => trans('messages.fail.action.cost', ['action' => 'delete', 'attribute' => 'Good Receipt', 'target' => 'Purchase Returns'], App::getLocale()),
            ]);
        }

        $goodReceipt->delete();

        return [
            'message' => trans('messages.success.delete', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Restore
     *
     * Restore the specified resource from storage.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function restore(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::onlyTrashed()->where('id', $request->route('id'))->firstOrFail();
        $goodReceipt->restore();

        return [
            'message' => trans('messages.success.restore', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Destroy
     *
     * Permanently remove the specified resource from storage.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function destroy(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::onlyTrashed()->where('id', $request->route('id'))->firstOrFail();
        $goodReceipt->forceDelete();

        return [
            'message' => trans('messages.success.destroy', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Approves
     *
     * Approve the specified resource.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function approve(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        $user = Auth::user();
        if (! $user) {
            throw ValidationException::withMessages([
                'user' => trans('messages.fail.action.cost', ['action' => 'approve', 'attribute' => 'Good Receipt', 'target' => 'Access'], App::getLocale()),
            ]);
        }
        $goodReceipt->approve($user);

        return [
            'message' => trans('messages.success.approve', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Reject
     *
     * Reject the specified resource.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function reject(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        $user = Auth::user();
        if (! $user) {
            throw ValidationException::withMessages([
                'user' => trans('messages.fail.action.cost', ['action' => 'reject', 'attribute' => 'Good Receipt', 'target' => 'Access'], App::getLocale()),
            ]);
        }
        $goodReceipt->reject($user);

        return [
            'message' => trans('messages.success.reject', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Cancel
     *
     * Cancel the specified resource.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function cancel(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        $user = Auth::user();
        if (! $user) {
            throw ValidationException::withMessages([
                'user' => trans('messages.fail.action.cost', ['action' => 'cancel', 'attribute' => 'Good Receipt', 'target' => 'Access'], App::getLocale()),
            ]);
        }
        $goodReceipt->cancel($user);

        return [
            'message' => trans('messages.success.cancel', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Rollback
     *
     * Roll back the specified resource.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function rollback(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        $user = Auth::user();
        if (! $user) {
            throw ValidationException::withMessages([
                'user' => trans('messages.fail.action.cost', ['action' => 'rollback', 'attribute' => 'Good Receipt', 'target' => 'Access'], App::getLocale()),
            ]);
        }
        $goodReceipt->rollback($user);

        return [
            'message' => trans('messages.success.rollback', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }

    /**
     * Good Receipt Force
     *
     * Force execute action on the specified resource.
     *
     * @return array{message: string, good_receipt: JsonResource}
     */
    public function force(Request $request): array
    {
        /** @var GoodReceipt $goodReceipt */
        $goodReceipt = GoodReceipt::where('id', $request->route('id'))->firstOrFail();

        $user = Auth::user();
        if (! $user) {
            throw ValidationException::withMessages([
                'user' => trans('messages.fail.action.cost', ['action' => 'force', 'attribute' => 'Good Receipt', 'target' => 'Access'], App::getLocale()),
            ]);
        }
        $goodReceipt->force($user, $request->input('step'));

        return [
            'message' => trans('messages.success.force', ['target' => 'Good Receipt'], App::getLocale()),
            'good_receipt' => $goodReceipt->toResource(),
        ];
    }
}
