...
class VendorPaymentComponentController extends Controller
{
     ...
     *
     * @return LengthAwarePaginator<int, VendorPaymentComponent>|Collection<int, VendorPaymentComponent>|JsonResource|int
     */
    public function index(Request $request): LengthAwarePaginator|Collection|JsonResource|int
    {
        ...
        if ($request->input('type', 'paginate') === 'collection') {
            return VendorPaymentComponentResource::collection($vendorPaymentComponents->get());
        }

        if ($request->input('type', 'paginate') === 'count') {
            return $vendorPaymentComponents->count();
        }

        return VendorPaymentComponentResource::collection($vendorPaymentComponents->withUsers()->paginate($request->input('per_page', 10), $request->input('columns', '*')));
    }
     ...
     *
     * @return array{message: string, vendor_payment_component: JsonResource}
     */
    public function store(Request $request): array
    {
        ...
        return [
            ...
            'vendor_payment_component' => $vendorPaymentComponent->toResource(),
        ];
    }
    ...
    public function update(Request $request): array
    {
        ...
        $vendorPaymentComponent = VendorPaymentComponent::where('id', $request->route('id'))->firstOrFail();
        $this->save($request, $vendorPaymentComponent);
    ...
    {
        /** @var VendorPaymentComponent $vendorPaymentComponent */
        $vendorPaymentComponent = VendorPaymentComponent::onlyTrashed()->where('id', $request->route('id'))->firstOrFail();
